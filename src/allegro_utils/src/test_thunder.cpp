...
void DummyNode::timer_callback(){
    ...
    kp.diagonal()  << 2*0.080, 2*0.080, 2*0.090, 2*0.080;
    kd.diagonal()  << 2*0.0035, 2*0.0035, 2*0.0015, 2*0.0007;

    // computed Torque law
    Eigen::MatrixXd tau_CT = M*(ddq_r + kp*e + kd*de) + C*dq.segment(4, 4) + G;

    Eigen::MatrixXd tau_PD = kp*e + kd*de + G + D*dq.segment(4, 4) + fs;//kp*e + kd*de; // to do: compensazione di gravità e attriti
