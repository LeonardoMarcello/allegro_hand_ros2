...
def generate_launch_description():
    ...
    declare_keyboard_arg = DeclareLaunchArgument(
        'KEYBOARD',
        default_value='false',
        description='Flag to enable/disable Keybord node'
    )
    ...
    return LaunchDescription([
        ...
        # Leo node
        #Node(
        #    package='allegro_utils',
        #    executable='moveit_joint_broadcaster',      # your script
        #    output='screen',
        #    parameters=[{
        #        'input_topic': '/joint_states',
        #        'output_topic': PythonExpression(["'allegroHand_",LaunchConfiguration('NUM'),"/joint_cmd'"]),
        #    }],
        #    condition=IfCondition(LaunchConfiguration('MOVEIT')),
        #),
        # Include WONIK GUI if GUI is true
		Node(
        ...
        # Include WONIK keyboard interface if KEYBOARD is true (Leo)
		Node(
            package='allegro_hand_keyboards',
            executable='allegro_hand_keyboard',
            output='screen',
            condition=IfCondition(LaunchConfiguration('KEYBOARD'))
        ),
        # Include the joint parser ti isaac if ISAAC is true (pezza di Leo )
		Node(
