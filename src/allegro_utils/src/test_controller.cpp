...
void DummyNode::timer_callback(){
    ...
    for (int i = 0; i < 16; ++i) {
      ...
      if (i>3 && i <=7){
        desired_position[i] = q_encoder(i-4);
      }else{
    ...
    for (int i = 0; i < 16; ++i) {
      ...
      pB_G(i) = pB_torque[i] - pB_kp[i]*(desired_position[i]-pB_current_position[i]) + pB_kd[i]*pB_current_velocity[i];
      joint_gc_cmd.position[i] = pB_current_position[i];
      joint_gc_cmd.effort[i] = pB_G(i);
    }
    ...
    */
    ...
    //Eigen::MatrixXd tau_PD = G + kp*e + kd*de; // to do: compensazione di gravità e attriti
